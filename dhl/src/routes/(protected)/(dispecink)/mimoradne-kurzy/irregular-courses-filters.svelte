<script lang="ts">
  ...
  async function onsubmit(e: Event & { currentTarget: HTMLFormElement }) {
    ...
    await irregularCoursesClient.find({
      filter: {
        ...
        load: data.has("load") ? (data.getAll("load") as string[]) : undefined,
      },
    });
  ...
  async function onreset() {
    irregularCoursesClient.find({ filter: undefined });
    stack.closeFilters();
  }
</script>
  ...
  <form {onsubmit} {onreset} method="post" id="dispatch-filters" novalidate>
    <FieldGrid cols={2} title="Vytvořeno" offset>
      <Input
        ...
        type="datetime-local"
        label="Od"
        value={formatDateTimeInput(irregularCoursesClient.filter?.createdAt?.[0])}
      />
      <Input
        id="dateTo"
        name="dateTo"
        type="datetime-local"
        label="Do"
        value={formatDateTimeInput(irregularCoursesClient.filter?.createdAt?.[1])}
      />
    </FieldGrid>
      ...
      <ChipGrid>
        {#each meta.value?.locations ?? [] as item (item.id)}
          <Chip
            ...
            checked={irregularCoursesClient.filter?.locationId?.includes(item.id)}
          >
            {item.name}
          </Chip>
        {/each}
      ...
      <ChipGrid>
        {#each ["hps", "obps", "ups"] ?? [] as item (item)}
          <Chip
            ...
            checked={irregularCoursesClient.filter?.network?.includes(item)}
          >
    ...
    <FieldGrid title="Zátěž" offset>
      <ChipGrid>
        <Chip
          id="load-filled"
          name="load"
          value="filled"
          faded={Boolean(search) && !"zadano".includes(unaccent(search))}
          checked={irregularCoursesClient.filter?.load?.includes("filled")}
        >
          Zadáno
        </Chip>
        <Chip
          ...
          name="load"
          value="empty"
          faded={Boolean(search) && !"nezadano".includes(unaccent(search))}
          checked={irregularCoursesClient.filter?.load?.includes("empty")}
        >
          Nezadáno
        </Chip>
      </ChipGrid>
