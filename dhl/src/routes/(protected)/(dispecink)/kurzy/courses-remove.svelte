<script lang="ts">
  ...
  let item = $derived(stack.remove ? coursesClient.get(Number(stack.remove)) : undefined);
  ...
  async function onsubmit() {
    ...
    try {
      await coursesClient.delete(item.value.id);
...
<Dialog onclose={() => stack.closeRemove()} title="Smazat záznam" offset>
  Smazat záznam {item?.value?.locationName} k datu {formatDate(item?.value?.departureDate)}?
